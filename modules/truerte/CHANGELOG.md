...
- Switched from a polynomial regular expression possibly creating a ReDoS vulnerabiliy to the native string trim method in truerte.utils.Tools.trim.

## 1.0.7 - 2025-01-11

### Fixed
- Fixed bundling TrueRTE together with a skin. (Tested with Vite/ES6 only.)
...
### Removed
- Slimmed README.md. Docs are now in the separate truerte/truerte-docs repo.
...
### Improved
- Updated references to bad TrueRTE 1.0.0 build in README.md #GH-23
...
- `TinyMCE`, `tinymce`, `Tinymce`, `TinyMce`, `tinyMCE`, `tinyMce` have been replaced by appropiate TrueRTE variants, especially the global `tinymce` object #GH-8
...
- The `promotion` init option has been removed. TrueRTE doesn't show an upgrade button regardless of it being present or not.
