...
import { EditorUiApi } from 'truerte/core/api/ui/Ui';
...
const setup = (editor: Editor, setupForTheme: ThemeRenderSetup): RenderInfo => {
  ...
  const renderDialogUi = () => {
    ...
    const sinkSpec = {
      dom: {
        tag: 'div',
        classes: [ 'tox', 'tox-silver-sink', 'tox-truerte-aux' ].concat(deviceClasses),
        attributes: {
          ...I18n.isRtl() ? { dir: 'rtl' } : {}
        }
      },
  ...
  const renderPopupUi = () => {
    ...
    const sinkSpec = {
      dom: {
        tag: 'div',
        classes: [ 'tox', 'tox-silver-sink', 'tox-silver-popup-sink', 'tox-truerte-aux' ].concat(deviceClasses),
        attributes: {
          ...I18n.isRtl() ? { dir: 'rtl' } : {}
        }
      },
  ...
  const renderMainUi = () => {
    ...
    const outerContainer = GuiFactory.build(
      OuterContainer.sketch({
        dom: {
          tag: 'div',
          classes: [ 'tox', 'tox-truerte' ]
            .concat(isInline ? [ 'tox-truerte-inline' ] : [])
            .concat(isToolbarBottom ? [ 'tox-truerte--toolbar-bottom' ] : [])
        ...
        behaviours: Behaviour.derive([
          ...
          Disabling.config({
            disableClass: 'tox-truerte--disabled'
          }),
          Keying.config({
            mode: 'cyclic',
