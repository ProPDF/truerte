...
import { renderCustomEditor } from 'truerte/themes/silver/ui/dialog/CustomEditor';

import * as RepresentingUtils from '../../../module/RepresentingUtils';

declare const truerte: TrueRTE;

describe('headless.truerte.themes.silver.components.customeditor.BasicCustomEditorTest', () => {
  const resolveInit = Cell(false);
  const customEditorValue = Cell('zztop');

  let origTiny: TrueRTE | undefined;
  before(() => {
    origTiny = Global.truerte;
    Global.truerte = {
      Resource
    };

    truerte.Resource.add('BasicCustomEditorTest', (e: HTMLElement) => new Promise((resolve) => {
