...
describe.skip('browser.truerte.themes.silver.window.SilverInlineDialogPositionTest', () => {
    ...
    Arr.each([
      ...
      context(tester.name, () => {
        ...
        it('Position of dialog should be constant when toolbar bottom docks', async () => {
          ...
          scrollRelativeEditor(editor, 'top', -100);
          editor.focus();
          await TinyUiActions.pWaitForPopup(editor, '.tox-truerte-inline');
          const dialog = openDialog(editor);
        ...
        context(editorType, () => {
          ...
          it('TINY-9888: Test bottom inline dialog position when scrolling', async () => {
            ...
            const assertFn = (dialog: SugarElement<HTMLElement>) => {
              ...
              const fixedSink = Class.has(TinyDom.container(editor), 'tox-truerte--toolbar-sticky-on');
