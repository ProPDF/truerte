...
import { ToolbarMode } from 'truerte/themes/silver/api/Options';

import { pOpenMore } from '../../module/MenuUtils';

// TODO TINY-10480: Investigate flaky tests
describe.skip('browser.truerte.themes.silver.editor.SilverInlineEditorWidthTest', () => {

  const structureTest = (editor: Editor, container: SugarElement<Node>, maxWidth: number) =>
    Assertions.assertStructure(
      'Container structure',
      ApproxStructure.build((s, str, arr) => s.element('div', {
        classes: [ arr.has('tox-truerte'), arr.has('tox-truerte-inline') ],
        children: [
          s.element('div', {
  ...
  const testRender = (options: RawEditorOptions, expectedWidth: number, pActions?: (editor: Editor) => Promise<void>) => async () => {
    ...
    Scroll.to(0, 500);
    await UiFinder.pWaitForVisible('Wait to be docked', SugarBody.body(), '.tox-truerte--toolbar-sticky-on .tox-editor-header');
    assertWidth(uiContainer, expectedWidth, expectedWidth - 100);
