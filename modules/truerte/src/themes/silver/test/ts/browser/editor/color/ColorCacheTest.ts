...
import { Menu } from 'truerte/core/api/ui/Ui';
import LocalStorage from 'truerte/core/api/util/LocalStorage';
import * as ColorCache from 'truerte/themes/silver/ui/core/color/ColorCache';

describe('browser.truerte.themes.silver.editor.color.ColorCacheTest', () => {
  afterEach(() => {
    LocalStorage.clear();
  ...
  it('TINY-9184: A cache should use legacy default if it exists', () => {
    LocalStorage.setItem('truerte-custom-colors', JSON.stringify([ '#0F00F0' ]));
    const colors = ColorCache.getCurrentColors('default');
