...
describe('browser.truerte.themes.silver.editor.ToolbarPersistTest', () => {
  ...
  it('TINY-4847: With toolbar_persist focus & unfocus should not affect toolbar visibility', async () => {
    ...
    await UiFinder.pWaitForHidden('Wait for editor to be hidden', SugarBody.body(), '.tox-truerte-inline');
    editor.focus();
    editor.nodeChanged();
    await Waiter.pWait(200); // Need to wait since nothing should happen.
    await UiFinder.pWaitForHidden('Wait for editor to be hidden', SugarBody.body(), '.tox-truerte-inline');
