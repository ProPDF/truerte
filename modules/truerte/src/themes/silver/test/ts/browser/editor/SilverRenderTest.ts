...
describe('browser.truerte.themes.silver.editor.SilverRenderTest', () => {
  ...
  const getRenderState = (editor: Editor): RenderState => {
    ...
    return {
      container: UiFinder.findIn(container, '.tox.tox-truerte').toOptional(),
      sink: UiFinder.findIn(container, '.tox.tox-truerte-aux').toOptional()
    };
  };
