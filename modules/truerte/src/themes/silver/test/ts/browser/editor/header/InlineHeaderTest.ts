...
describe('browser.truerte.themes.silver.editor.header.InlineHeaderTest', () => {
  ...
  const pAssertHeaderWidth = (expectedWidth: string, expectedMaxWidth: string) =>
    Waiter.pTryUntil('Could not verify width', () => {
      ...
      const headerWrapper = SelectorFind.descendant(SugarBody.body(), '.tox-truerte--toolbar-sticky-off').getOrDie();
