...
describe('browser.truerte.plugins.emoticons.DifferentEmojiDatabaseTest', () => {
  before(() => {
    Plugin();
  });
  ...
  const pTestEditorWithSettings = async (categories: string[], databaseUrl: string) => {
    const editor = await McEditor.pFromSettings<Editor>({
      ...
      emoticons_database_id: 'truerte.plugins.emoticons.' + getFilename(databaseUrl)
    });
  ...
  it('TBA: Loading databases from different urls ', async () => {
    await pTestEditorWithSettings([ 'All', 'People' ], '/project/truerte/src/plugins/emoticons/test/js/test-emojis.js');
    await pTestEditorWithSettings([ 'All', 'Travel and Places' ], '/project/truerte/src/plugins/emoticons/test/js/test-emojis-alt.js');
  });
});
