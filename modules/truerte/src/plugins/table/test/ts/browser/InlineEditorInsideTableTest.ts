...
describe.skip('browser.truerte.plugins.table.InlineEditorInsideTableTest', () => {
  const setupElement = () => {
    const containerHtml = '<table>' +
      '<tbody>' +
      '<tr>' +
      '<td>' +
      '<div class="truerte" style="border: 1px gray solid">a</div>' +
      '</td>' +
      '</tr>' +
      '</tbody>' +
      '</table>';
    ...
    const editorDiv = SelectorFind.descendant<HTMLDivElement>(container, 'div.truerte').getOrDie('Cannot find TrueRTE div');
    return {
      element: editorDiv,
