...
describe('browser.truerte.plugins.help.DialogKeyboardNavTest', () => {
  ...
  it('TBA: test the tab key navigation cycles through all focusable fields in Version tab', async () => {
    ...
    pressTabKey(editor);
    await pAssertFocusOnItem('TrueRTE Version', 'div[role="document"]');
    pressTabKey(editor);
    await pAssertFocusOnItem('Installed Plugins', `a[href*="truerte"]`);
    pressTabKey(editor);
    await pAssertFocusOnItem('Close Button', '.tox-button:contains("Close")');
    pressTabKey(editor);
    await pAssertFocusOnItem('"x" Close Button', '.tox-button[data-mce-name="close"]');
