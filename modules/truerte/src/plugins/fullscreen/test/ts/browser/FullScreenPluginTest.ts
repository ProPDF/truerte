...
import { NotificationApi } from 'truerte/core/api/NotificationManager';
import FullscreenPlugin from 'truerte/plugins/fullscreen/Plugin';
import LinkPlugin from 'truerte/plugins/link/Plugin';

describe('browser.truerte.plugins.fullscreen.FullScreenPluginTest', () => {
  ...
  const assertEditorContainerAndSinkState = (editor: Editor, shouldExist: boolean) => {
    ...
    const sink = UiFinder.findIn(contentContainer, '.tox-silver-sink.tox-truerte-aux').getOrDie();
