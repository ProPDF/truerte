...
import { TrueRTE } from '../api/Truerte';
...
const makeStylesheetLoadingPromises = (editor: Editor, css: string[], framedFonts: string[]): Promise<unknown>[] => {
  const { pass: bundledCss, fail: normalCss } = Arr.partition(css, (name) => truerte.Resource.has(name));
  const bundledPromises = bundledCss.map((url) => {
    const css = truerte.Resource.get(url);
