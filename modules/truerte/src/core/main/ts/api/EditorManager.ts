...
declare const window: Window & { truerte?: PreInit; trueRTEPreInit?: PreInit };
...
const EditorManager: EditorManager = {
  ...
  /**
   * Major version of TrueRTE build.
   *
  ...
  /**
   * Minor version of TrueRTE build.
   *
   ...
   * @type truerte.Editor
   * @example
   * trueRTE.activeEditor.selection.getContent();
   * truerte.EditorManager.activeEditor.selection.getContent();
   */
  activeEditor: null,
  focusedEditor: null,
  ...
  setup() {
    ...
    // If truerte is defined and has a base use that or use the old trueRTEPreInit
    const preInit = window.truerte || window.trueRTEPreInit;
    if (preInit) {
      baseURL = preInit.base || preInit.baseURL;
      suffix = preInit.suffix;
    } else {
      // Get base where the truerte script is located
      ...
      for (let i = 0; i < scripts.length; i++) {
        ...
        // truerte.js truerte.min.js truerte.dev.js
        // truerte.jquery.js truerte.jquery.min.js truerte.jquery.dev.js
        // truerte.full.js truerte.full.min.js truerte.full.dev.js
        const srcScript = src.substring(src.lastIndexOf('/'));
        if (/truerte(\.full|\.jquery|)(\.min|\.dev|)\.js/.test(src)) {
    ...
    /**
     * Base URL where the root directory if TrueRTE is located.
     *
     * @property baseURL
     * @type String
     */
    ...
    /**
     * Absolute baseURI for the installation path of TrueRTE.
     *
     * @property baseURI
   ...
   * <br /><br />
   * Therefore, when using <code>overrideDefaults</code> with the cloud-based editor, any newly integrated options must be combined with the options in <code>truerte.defaultOptions</code>.
   *
   ...
   * };
   *
   * truerte.overrideDefaults({
   *   ...truerte.defaultOptions,
   *   ...customOptions
   * });
   */
   ...
   * <br /><br />
   * For information on basic usage of <code>init</code>, see: <a href="https://www.truerte.org/docs/truerte/1/basic-setup/">Basic setup</a>.
   *
   * @method init
    ...
    const findTargets = (options: RawEditorOptions): HTMLElement[] => {
      if (Env.browser.isIE() || Env.browser.isEdge()) {
        ErrorReporter.initError(
          'TrueRTE does not support the browser you are using. For a list of supported' +
          ' browsers please see: https://www.truerte.org/docs/truerte/1/support/#supportedwebbrowsers'
        );
        return [];
      } else if (isQuirksMode) {
        ErrorReporter.initError(
          'Failed to initialize the editor as the document is not in standards mode. ' +
          'TrueRTE requires standards mode.'
        );
        return [];
   ...
   * @return {truerte.Editor/Array} Editor instance or an array of editor instances.
   * @example
   * // Adds an onclick event to an editor by id
   * truerte.get('mytextbox').on('click', (e) => {
   *   ed.windowManager.alert('Hello world!');
   * });
   *
   * // Adds an onclick event to an editor by index
   * truerte.get(0).on('click', (e) => {
   *   ed.windowManager.alert('Hello world!');
   * });
   *
   * // Adds an onclick event to an editor by id (longer version)
   * truerte.EditorManager.get('mytextbox').on('click', (e) => {
   *   ed.windowManager.alert('Hello world!');
   * });
   */
   ...
   *
   * @method add
   * @param {truerte.Editor} editor Editor instance to add to the collection.
   * @return {truerte.Editor} The same instance that got passed in.
   */
  add(editor) {
    const self: EditorManager = this;
   ...
   * @return {truerte.Editor} Editor instance that got created.
   */
   ...
   *
   * @method remove
   * @param {truerte.Editor/String/Object} [selector] CSS selector or editor instance to remove.
   * @return {truerte.Editor} The editor that got passed in will be return if it was found otherwise null.
   */
   ...
   *
   * @method setActive
   * @param {truerte.Editor} editor Editor instance to set as the active instance.
   */
  setActive(editor) {
