 ...
 *
 * @example
 * const parser = truerte.html.DomParser({ validate: true }, schema);
 * const rootNode = parser.parse('<h1>content</h1>');
 *
 * @class truerte.html.DomParser
 * @version 3.4
 */
...
const DomParser = (settings: DomParserSettings = {}, schema = Schema()): DomParser => {
   ...
   * @return {truerte.html.Node} Root node containing the tree.
   * @example
   * const rootNode = truerte.html.DomParser({...}).parse('<b>text</b>');
   */
