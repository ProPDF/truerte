 ...
 * Consult the TrueRTE API Documentation for more details and examples on how to use this class.
 *
 * @class truerte.dom.Selection
 * @example
 * // Getting the currently selected node for the active editor
 * alert(truerte.activeEditor.selection.getNode().nodeName);
 */
 ...
 * @constructor
 * @method Selection
 * @param {truerte.dom.DOMUtils} dom DOMUtils object reference.
 * @param {Window} win Window to bind the selection object to.
 * @param {truerte.dom.Serializer} serializer DOM serialization class to use for getContent.
 * @param {truerte.Editor} editor Editor instance of the selection.
 */
const EditorSelection = (dom: DOMUtils, win: Window, serializer: DomSerializer, editor: Editor): EditorSelection => {
   ...
   * alert(truerte.activeEditor.selection.getContent());
   *
   * // Alerts the currently selected contents as plain text
   * alert(truerte.activeEditor.selection.getContent({ format: 'text' }));
   */
   ...
   * truerte.activeEditor.selection.setContent('<strong>Some contents</strong>');
   */
   ...
   * const bm = truerte.activeEditor.selection.getBookmark();
   *
   * truerte.activeEditor.setContent(truerte.activeEditor.getContent() + 'Some new content');
   *
   * // Restore the selection bookmark
   * truerte.activeEditor.selection.moveToBookmark(bm);
   */
   ...
   * truerte.activeEditor.selection.select(truerte.activeEditor.dom.select('p')[0]);
   */
  ...
  const getRng = (): Range => {
    ...
    try {
      ...
      // IE throws unspecified error here if TrueRTE is placed in a frame/iframe
    }
   ...
   * truerte.activeEditor.selection.setNode(truerte.activeEditor.dom.create('img', { src: 'some.gif', title: 'some title' }));
   */
