...
const logRemovedWarnings = (rawOptions: RawEditorOptions, normalizedOptions: NormalizedEditorOptions): void => {
  ...
  if (hasRemovedPlugins || hasRemovedOptions || isLegacyMobileTheme) {
    ...
    console.warn(
      'The following deprecated features are currently enabled but have been removed in TinyMCE 6.0 and therefore, they are not present in TrueRTE either. These features will no longer work and should be removed from the TrueRTE configuration. ' +
      'See https://www.truerte.org/docs/truerte/1/migration-from-5x/ for more information.' +
      themesMessage +
      pluginsMessage +
      optionsMessage
    );
