...
import VisualBlocksPlugin from 'truerte/plugins/visualblocks/Plugin';

describe('browser.truerte.core.EditorCleanupTest', () => {
  before(() => {
    VisualBlocksPlugin();
  });
  ...
  const testCleanup = (comment: string, settings: RawEditorOptions, html: string = '<div></div>', fn: (editor: Editor) => void = Fun.noop) => {
    it(comment, async () => {
      ...
      const editor = await McEditor.pFromHtml<Editor>(html, { base_url: '/project/truerte/js/truerte', ...settings });
  ...
  testCleanup(
    ...
    '<div></div>',
    (editor) => {
      editor.dom.loadCSS('/project/truerte/js/truerte/skins/ui/dark/skin.css');
    });

  // Loaded via StyleSheetLoader
  assertPageLinkPresence('/project/truerte/js/truerte/skins/ui/content/default/content.inline.css', false);
  assertPageLinkPresence('/project/truerte/js/truerte/skins/ui/oxide/skin.css', false);
  // Loaded via DOMUtils as per above
  assertPageLinkPresence('/project/truerte/js/truerte/skins/ui/dark/skin.css', false);
});
