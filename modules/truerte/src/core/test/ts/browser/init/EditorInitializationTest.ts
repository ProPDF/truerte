...
describe('browser.truerte.core.init.EditorInitializationTest', () => {
  ...
  const pCreateInlineModeMultipleInstances = (): Promise<Editor[]> => {
    viewBlock.update('<div class="truerte-editor"><p>a</p></div><div class="truerte-editor"><p>b</p></div>');

    return EditorManager.init({
      selector: '.truerte-editor',
      inline: true,
      toolbar_mode: 'wrap'
    });
  };
  ...
  const assertEditors = (editors: Editor[]) => {
    ...
    const containerApproxStructure = ApproxStructure.build((s, str, arr) => s.element('div', {
      classes: [ arr.has('tox'), arr.has('tox-truerte'), arr.has('tox-truerte-inline') ],
      children: [
        s.element('div', {
