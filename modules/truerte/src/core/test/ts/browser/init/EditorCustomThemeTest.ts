...
import { Theme } from 'truerte/core/api/ThemeManager';
import { TrueRTE } from 'truerte/core/api/Truerte';

declare const truerte: TrueRTE;

describe('browser.truerte.core.init.EditorCustomThemeTest', () => {
  ...
  context('named synchronous custom theme', () => {
    ...
    const SyncTheme = () => {
      truerte.ThemeManager.add('sync_theme', (editor): Theme => {
        const theme: Theme = {
          renderUI: () => {
            const editorContainer = document.createElement('div');
