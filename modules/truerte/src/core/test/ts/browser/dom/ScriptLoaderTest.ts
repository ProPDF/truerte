...
import ScriptLoader from 'truerte/core/api/dom/ScriptLoader';

describe('browser.truerte.core.dom.ScriptLoaderTest', () => {
  const testScript = '/project/truerte/src/core/test/assets/js/test.js';
  const nestedScript = '/project/truerte/src/core/test/assets/js/nested.js';
  const invalidScript = '/project/truerte/src/core/test/assets/js/invalid.js';
  let loadedCount = 0;

  before(() => {
    Global.truerte_ = { ScriptLoader: ScriptLoader.ScriptLoader };
  });

  after(() => {
    delete Global.truerte_;
  });
