...
describe('browser.truerte.core.EditorRemoveTest', () => {
  ...
  const testRemoveStyles = (editor: Editor, expectedStyle: string) => {
    ...
    EditorManager.init({ selector: '#truerte' });
    assertTextareaDisplayStyle(editor, expectedStyle);
  ...
  it('init editor with no display style', async () => {
    const editor = await McEditor.pFromHtml<Editor>('<textarea id="truerte"></textarea>', settings);
    testRemoveStyles(editor, '');
  });

  it('init editor with display: none', async () => {
    const editor = await McEditor.pFromHtml<Editor>('<textarea id="truerte" style="display: none;"></textarea>', settings);
    testRemoveStyles(editor, 'none');
  });

  it('init editor with display: block', async () => {
    const editor = await McEditor.pFromHtml<Editor>('<textarea id="truerte" style="display: block;"></textarea>', settings);
