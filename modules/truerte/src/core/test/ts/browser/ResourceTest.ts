...
import Resource from 'truerte/core/api/Resource';

declare const truerte: { Resource: Resource };

describe('browser.truerte.core.ResourceTest', () => {
  const origTiny = Global.truerte;
  ...
  after(() => {
    Global.truerte = origTiny;
  });

  const testScript = (id: string, data: string) => `data:text/javascript,truerte.Resource.add('${id}', '${data}')`;

  const addScript = (id: string, data: string) => {
    truerte.Resource.add(id, data);
  };

  const loadScript = (id: string, url: string): Promise<any> =>
    truerte.Resource.load(id, url);

  const unloadScript = (id: string) => {
    truerte.Resource.unload(id);
  };
  ...
  it('invalid id fails', () => {
    ...
    return assertLoadFailure(load, `Script at URL "data:text/javascript,truerte.Resource.add('invalid-id', 'value.4')" did not call \`truerte.Resource.add('script.4', data)\` within 1 second`);
  });
