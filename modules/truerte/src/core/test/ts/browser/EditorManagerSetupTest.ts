...
const storeState = () => {
  ...
  const origTrueRTE = (window as any).truerte;
  delete (window as any).truerte;
  ...
  return () => {
    ...
    (window as any).truerte = origTrueRTE;
  };
};

describe('browser.truerte.core.EditorManagerSetupTest', () => {
  ...
  it('script baseURL and suffix with script in svg', () => {
    viewBlock.update('<svg><script>!function(){}();</script></svg><script src="http://localhost/nonexistant/truerte.min.js" type="application/javascript"></script>');
    EditorManager.setup();
