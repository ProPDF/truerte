...
import * as Options from 'truerte/core/api/Options';

describe('browser.truerte.core.api.OptionsTest', () => {
  it('getBodyId', () => {
    assert.equal(Options.getBodyId(new Editor('id', {}, EditorManager)), 'truerte', 'Should be default id');
    ...
    assert.equal(Options.getBodyId(new Editor('idc', { body_id: 'ida=a,idb=b' }, EditorManager)), 'truerte', 'Should be default id for idc');
  });
