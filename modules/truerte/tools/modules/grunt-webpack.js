    ...
    plugins: [
      ...
      new webpack.WatchIgnorePlugin({
        paths: [
          ...
          /truerte\/modules\/.*\/lib\/.*\.(js|map|d\.ts)$/,
...
const all = (plugins, themes, models) => {
  return [
    create({
      ...
      ...buildEntries(plugins, 'plugins', 'Main.ts', 'js/truerte/'),
      ...buildDemoEntries(models, 'models', 'Demo.ts', 'scratch/demos/'),
      ...buildEntries(models, 'models', 'Main.ts', 'js/truerte/'),
      ...buildDemoEntries(themes, 'themes', 'Demos.ts', 'scratch/demos/'),
      ...buildEntries(themes, 'themes', 'Main.ts', 'js/truerte/'),
    }, '../../tsconfig.demo.json'),
    // Note: This can't use the demo tsconfig as it is the core package
    create({
      'js/truerte/truerte.js': 'src/core/main/ts/api/Main.ts'
    }, '../../tsconfig.json')
  ];
};
