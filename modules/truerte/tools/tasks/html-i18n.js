...
const resourceBase = 'truerte.html-i18n';
...
const createTrueRTEJs = (translation, resourceName) =>
  `truerte.Resource.add('${resourceName}',\n${util.inspect(translation)});`
...
const generate = (grunt, srcDir, outputDir, resourcePrefix) => {
  findFiles(srcDir).forEach((file) => {
    ...
    const content = createTrueRTEJs(translation, `${resourcePrefix}.${langCode}`);
