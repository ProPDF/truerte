...
* external api configs provided by users though api calls to the public truerte api
* internal validated variants of those configs

The idea is that TrueRTE core is developed against the internal strict configuration objects to simplify the code. At the same time, developers of TrueRTE plugins have a nice easy-to-use API that requires minimal configuration.
...
* This package should not know about truerte, truerte legacy ui or alloy it's simply a bunch of validators external data and interfaces.
* The public structure is somewhat similar to what is in tinymce 4.x to make migration easy for end users.

# Usage

Bridge is not designed to be used directly; it forms an API layer as part of the TrueRTE User Interface.
